...
/* ---------------------------------------------------------------------------
   APAGADO — 2026-08-07, a petición de Jihan: los compuestos vuelven a su
   nombre original.

   Se apaga, no se borra. El trabajo de arriba —el criterio, la razón de cada
   nombre, la regla de que ninguno prometa nada— cuesta más de rehacer que de
   guardar, y ponerlo a `true` lo devuelve entero. Mientras esté en `false`,
   nada de este fichero toca una sola tarjeta: BPC-157 se llama BPC-157.
   --------------------------------------------------------------------------- */
...
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
/* La salida va aquí y no antes: así el mapa NAMES de arriba se sigue leyendo
   y se sigue pudiendo consultar desde `window.__pxNames`, pero no se engancha
   nada al sitio. La clase que gobierna el CSS se retira explícitamente, para
   que apagar esto no dependa de que el documento arranque limpio. */
if(!NAMES_ON){
  document.documentElement.classList.remove('px-name-leads', 'px-names');
  window.__pxNames = {map: NAMES, of: n => n, leads: false, on: false};
  return;
}
