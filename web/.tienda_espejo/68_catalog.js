...
const PX_CAT = [
  ...
  /* Las plumas pasan de NUEVO a PRONTO. «Nuevo» promete algo que se puede
     pedir hoy; hasta que haya catálogo, lo honesto es PRONTO. */
...
function fillMenu(){
  const m = buildMenu();
  /* EL LOGOTIPO EN LUGAR DEL NOMBRE, NO AL LADO.
     `badge` llevaba declarado desde el principio sin usarse, y por un motivo
     que sigue siendo cierto: el lockup de línea es una pieza de casi 5:1, y
     metido como icono junto al texto se convierte en una mancha gris.
     La salida no es encogerlo hasta que quepa — es darle el ancho de la fila y
     quitar el nombre en texto, que decía exactamente lo mismo que el logotipo.
     El lockup ya ES el nombre. Debajo queda el descriptor, que sí añade algo.
     Las secciones sin logotipo —plumas, accesorios— conservan su nombre en
     texto, compuesto para ocupar la misma altura. */
  const marca = c => {
    const src = c.badge && typeof LOGOS === 'object' && LOGOS[c.badge];
    return src
      ? '<img class="lg" src="' + src + '" alt="' + esc(nameOf(c)) + '" loading="lazy"/>'
      : '<span class="nm">' + esc(nameOf(c)) + '</span>';
  };

  const row = c => {
    const soon = !!c.soon;
    return '<a href="#' + c.route + '" data-nav ' +
           'data-href="#' + c.route + '" role="menuitem" class="' +
           (soon ? 'soon' : '') + (c.badge ? ' conlogo' : '') + '">' +
           '<span class="tx">' + marca(c) +
