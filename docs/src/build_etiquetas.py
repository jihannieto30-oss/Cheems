...
/* ---------- estado ---------- */
function load(){
  try{ var d=JSON.parse(localStorage.getItem(KEY)); if(d&&typeof d==='object') edits=d; }catch(e){}
  migrate();
}

/* UN SOLO CUBO POR PRESENTACIÓN, NO UNO POR TAMAÑO

   Las ediciones se guardaban con la clave `compuesto|50x30`, así que lo que
   escribías en una medida no existía en la otra: había que teclear el lote y
   la caducidad dos veces, y al cambiar de tamaño parecía que se hubieran
   borrado. Tenía sentido cuando las dos medidas eran dos diseños distintos.
   Ahora son el mismo diseño a dos escalas, y el contenido es el mismo
   contenido.

   Lo que ya estuviera guardado no se pierde: se funde en la clave sin tamaño.
   Si hay de las dos, manda la de 50×30 — es la que está por defecto y la que
   suele llevar todo relleno — y lo que sólo exista en la pequeña se rellena
   encima. */
function migrate(){
  var moved=false, out={};
  Object.keys(edits).forEach(function(k){
    var bar=k.indexOf('|');
    if(bar<0){ out[k]=edits[k]; return; }
    moved=true;
    var base=k.slice(0,bar), win=k.slice(bar+1)==='50x30';
    if(!out[base]) out[base]={};
    var src=edits[k];
    for(var f in src){
      if(win || out[base][f]===undefined || out[base][f]==='') out[base][f]=src[f];
    }
  });
  if(moved){ edits=out; save(); }
}
function save(){ try{ localStorage.setItem(KEY, JSON.stringify(edits)); }catch(e){} }
function keyOf(i){ return ITEMS[i].k; }
...
/* ============================ EL DIBUJO ============================
   ...
   UN SOLO DISEÑO, DOS TAMAÑOS

   Antes había veintitantos «si es la pequeña, entonces» repartidos por la
   rutina, y eso no son dos tamaños de un diseño: son dos diseños que se
   parecen.  Se nota justo donde más duele — el lema aparecía en la de 50×30 y
   desaparecía en la de 40×20, porque se dibujaba entero o nada y en la
   pequeña nunca cabía.

   Ahora hay una sola composición y un solo número, k, que es la raíz del
   cociente de áreas:

       k = √( (472·236) / (591·354) ) = 0.73

   Todo lo que ocupa sitio se multiplica por k.  El tipo también, pero con
   suelo: por debajo de cierto cuerpo la térmica cierra los contratipos y da
   igual lo que diga la geometría.  Así las dos etiquetas son la misma, una a
   escala de la otra, y el lema está en las dos porque tiene su propia línea.

   LA COMPOSICIÓN, Y DE DÓNDE SALE

   Del lockup entregado, que se lee así:
   ...
   Marca, filete, línea, lema.  La etiqueta repite esa estructura en vez de
   inventar otra:

       [icono]Px  P E P T I D E X          ▓▓▓
                  F I T N E S S            ▓▓▓
                                           ▓▓▓
       BPC-157                             ▓▓▓
       ──────────
       1 0   M G
       ─────────────────────────────────────────
       BUILD PERFORMANCE      RESEARCH USE ONLY
       LOTE 2601-BC-01 · EXP 06.27

   El filete corto bajo el nombre del compuesto es el mismo gesto que el filete
   bajo PEPTIDEX en el lockup.  Es lo que hace que la etiqueta se lea como
   parte de la marca y no como una pegatina de inventario.

   LO QUE SE HA QUITADO

     el recuadro negro de la dosis   Un bloque relleno es lo contrario de esta
                                     marca, que son filetes finos y letras muy
                                     espaciadas sobre blanco.  La dosis pasa a
                                     texto espaciado bajo su filete: se
                                     encuentra igual de rápido y pesa la mitad.
     el filete de la cabecera        El aire ya separa.  Un filete que no
                                     separa nada sólo llena.

   Quedan dos filetes en toda la etiqueta y ninguna mancha negra.  Se dibuja en
   gris y se umbraliza al final: dibujar ya en blanco y negro obliga a
   renunciar al antialias, y sin antialias el trazado del navegador se come los
   remates.                                                                  */
function draw(){
  ...
  /* el único número que distingue un tamaño del otro */
  var k = Math.sqrt((W*H)/(mm(50)*mm(30)));

  /* Medida en milímetros escalada.  `floor` es el mínimo en milímetros por
     debajo del cual la térmica cierra el trazo: la escala nunca lo cruza. */
  function u(base, floor){
    var v = mm(base)*k;
    if(floor!==undefined && v<mm(floor)) v=mm(floor);
    return Math.round(v);
  }

  var pad   = u(1.8, 1.15);
  var RULE  = Math.max(3, Math.round(mm(0.26)*k));
  var fullW = W-pad*2;

  /* --- 1 · LA FRANJA INFERIOR SE MIDE ANTES QUE NADA ----------------------
     Su altura no depende de nada más de la etiqueta, así que fijarla primero
     da el techo real del que dispone todo lo demás — el QR incluido.  Medirla
     al final, como estaba, es lo que hacía que el filete acabara cruzando el
     QR en cuanto había lote y caducidad. */
  var fsFoot = u(1.45, 1.35);
  var gFoot  = u(0.7, 0.45);
  var le=[];
  ...
  var bottomH = footLines ? footLines*fsFoot + (footLines-1)*gFoot
                            + RULE + u(1.0, 0.7) : 0;
  var ceil = H-pad-bottomH;              // nada de arriba baja de aquí

  /* --- 2 · EL QR, DENTRO DEL HUECO QUE QUEDA -----------------------------
     Centrado contra [pad, ceil] y no contra el lienzo: así nunca se mete
     debajo del filete ni pisa la franja legal.
     El módulo se redondea a entero — uno fraccionario reparte medio píxel
     entre dos celdas y el lector pierde la retícula. */
  var qrBox=0, qrX=0, qrY=0, mod=0, qm=null;
  if(d.qrOn!==false && typeof QRGen!=='undefined'){
    try{ qm=QRGen.matrix(String(d.qr||" ")); }catch(e){ qm=null; }
  }
  if(qm){
    var quiet=4, n=qm.length, tot=n+quiet*2;
    /* No pasa de un tercio del ancho: por encima de eso el nombre del
       compuesto se queda sin sitio y hay que encogerlo hasta que no se lee.
       El módulo entre 4 y 8 px — por debajo de 4 no lo lee un teléfono, y por
       encima de 8 sólo se gana tamaño, no legibilidad. */
    var room = ceil-pad;
    var maxBox = Math.min(room, Math.round(W*0.34));
    mod = Math.floor(maxBox/tot);
    if(mod>8) mod=8;
    if(mod<4) mod = Math.max(4, Math.min(8, Math.floor(room/tot)));
    qrBox=tot*mod;
    qrX=W-pad-qrBox; qrY=pad+Math.round((room-qrBox)/2);
    if(qrY<pad) qrY=pad;
  ...
  var right = qrBox ? qrX-u(1.6,1.0) : W-pad;
  var colW  = right-pad;

  /* --- 3 · CABECERA: marca, casa y línea ---------------------------------
     La estructura del lockup entregado — [icono]Px / PEPTIDEX / línea — puesta
     en horizontal para que quepa. */
  var y=pad, headH=0;
  var lg=imgs[d.line];
  ...
  if(d.logo!=='0' && lg && lg.width){
    var lh = u(d.logo==='2'?5.4:4.4, 2.6);
    var lw = Math.round(lg.width*lh/lg.height);
    if(lw>colW*0.42){ lw=Math.round(colW*0.42); lh=Math.round(lg.height*lw/lg.width); }
  ...
  /* --- 4 · NOMBRE, FILETE Y DOSIS ---------------------------------------- */
  var fsDose = u(2.5, 1.75);
  var fsSub  = u(1.5, 1.20);
  var gapR   = u(0.55, 0.35);
  var hasDose= !!String(d.dose||'');
  var doseH  = hasDose ? gapR+RULE+gapR+fsDose : 0;
  var subH   = d.sub ? u(0.6,0.4)+fsSub : 0;

  var nameMax = u(6.6, 2.6);
  var nameMin = u(3.0, 2.2);
  var free = ceil - y - doseH - subH;
  if(free < nameMax) nameMax = Math.max(nameMin, free);

  /* EL NOMBRE NO SE RECORTA

     «CJC-1295 + IPAMO…» en un vial no es un nombre: es un fallo. Un compuesto
     que no cabe en una línea cabe en dos — son dos palabras — y en dos líneas
     el cuerpo puede ser mayor, no menor. Así que primero se intenta una línea;
     si no entra sin bajar del mínimo legible, se parte por el hueco que deje
     las dos mitades más parejas y se recompone. Recortar queda como último
     recurso y con aviso.

     Y «condensado» pasa a condensar de verdad. Antes pedía la familia
     "Arial Narrow", que en muchos equipos no está instalada: el navegador caía
     en Arial y la opción no hacía absolutamente nada. Ahora es una escala
     horizontal del lienzo, que existe en todas partes y hace exactamente lo
     que dice el nombre de la casilla. */
  var fam = 'Arial,Helvetica,sans-serif';
  var sx  = d.cond==='1' ? 0.84 : 1;
  var nm  = String(d.name||'').toUpperCase();
  var gapL = Math.round(u(0.3,0.2));

  function nw(t, s){ ctx.font='800 '+s+'px '+fam; return ctx.measureText(t).width*sx; }
  function fitLines(arr, hi){
    for(var s=hi; s>=nameMin; s--){
      var ok=true;
      for(var i=0;i<arr.length;i++) if(nw(arr[i],s)>colW){ ok=false; break; }
      if(ok) return s;
    }
    return 0;
  }
  ...
  function splitTwo(t){
    ...
    for(var i=1;i<w.length;i++){
      var a=w.slice(0,i).join(' '), b=w.slice(i).join(' ');
      var sc=Math.abs(a.length-b.length);
      if(sc<score){ score=sc; best=[a,b]; }
    }
    return best;
  }

  var lines=[nm], fs=fitLines([nm], nameMax);
  if(!fs){
    var two = splitTwo(nm);
    var f2 = two ? fitLines(two, Math.min(nameMax, Math.floor(free/2.08))) : 0;
    if(f2){ lines=two; fs=f2; }
    else { fs=nameMin; }          /* ni así: se recorta y se avisa */
  }
  var clipped = false;
  var nameH = fs*lines.length + gapL*(lines.length-1);

  /* El bloque central se centra en el hueco que le dejan la cabecera y el pie.
     Colgado del borde de arriba, como estaba, todo el aire sobrante se
     acumulaba en un solo sitio y la etiqueta grande parecía a medio hacer. */
  var bodyH = nameH + doseH + subH;
  y += Math.max(0, Math.round((ceil - y - bodyH)/2));

  var nameW = 0;
  ctx.font='800 '+fs+'px '+fam;
  for(var li=0; li<lines.length; li++){
    var txt = lines[li];
    if(nw(txt,fs) > colW){ txt = clip(txt, colW/sx, fs, '800'); clipped = true; }
    y += fs;
    ctx.save(); ctx.translate(pad, 0); ctx.scale(sx, 1);
    ctx.font='800 '+fs+'px '+fam;
    ctx.fillText(txt, 0, y);
    ctx.restore();
    nameW = Math.max(nameW, Math.min(colW, ctx.measureText(txt).width*sx));
    if(li<lines.length-1) y += gapL;
  }

  /* El filete corto bajo el nombre es el mismo gesto que el filete bajo
     PEPTIDEX en el lockup: al ancho del nombre, no al de la columna. */
  if(hasDose){
    y += gapR;
    ctx.fillRect(pad, y, Math.max(nameW, u(8,5)), RULE);
    y += RULE+gapR+fsDose;
    ctx.font='700 '+fsDose+'px Arial,Helvetica,sans-serif';
    /* «5mg» se escribe pegado y se lee peor: la cifra y la unidad son dos
       cosas. Se separan al dibujar, sin tocar lo que hay escrito en el campo. */
    var dz=String(d.dose).toUpperCase().replace(/(\d)\s*(MG|MCG|ML|IU|G)\b/g,'$1 $2');
    spDraw(dz, pad, y, spFit(dz, colW, fsDose*0.16));
  }

  if(d.sub){
    y += u(0.6,0.4)+fsSub;
    ctx.font='700 '+fsSub+'px Arial,Helvetica,sans-serif';
    ctx.fillText(clip(String(d.sub), colW, fsSub, '700'), pad, y);
  }

  /* --- 5 · FRANJA INFERIOR, A ANCHO COMPLETO ----------------------------- */
  if(footLines){
    var fy = H-pad;
    ctx.font='700 '+fsFoot+'px Arial,Helvetica,sans-serif';
    if(lotLine){
...
/* INTERLETRADO DE VERDAD

   Antes esto metía un espacio fino (U+200A) entre cada letra, y no funcionaba:
   el espacio fino mide una fracción de pica y a cuerpo de etiqueta desaparece.
   El resultado era «PEPTIDEX» apretado donde el lockup pone P E P T I D E X, y
   ese espaciado no es un detalle de la marca — es la marca.

   ctx.letterSpacing existe, pero no en todo lo que nos importa. Así que se
   dibuja letra a letra y se avanza a mano. Es exacto en cualquier navegador y
   permite medir antes de dibujar, que es lo que hace falta para que quepa. */
function spW(txt, tr){
  var w=0;
  for(var i=0;i<txt.length;i++) w += ctx.measureText(txt[i]).width + tr;
  return w - (txt.length?tr:0);
}
function spDraw(txt, x, y, tr){
  var w=0;
  for(var i=0;i<txt.length;i++){
    ctx.fillText(txt[i], x+w, y);
    w += ctx.measureText(txt[i]).width + tr;
  }
  return w - (txt.length?tr:0);
}
/* Cabe o se aprieta: primero se cede interletrado, que es lo que menos duele,
   y sólo si aun así no entra se recorta. Encoger el cuerpo aquí no sirve —
   estas líneas ya van al mínimo legible. */
function spFit(txt, maxW, tr0){
  var tr=tr0;
  while(tr>0 && spW(txt,tr)>maxW) tr -= 0.5;
  if(tr<0) tr=0;
  return tr;
}
...
function checkWarnings(clipped, mod, nLines, fsFoot, tagW, fullW){
  var w=document.getElementById('warn'), msgs=[];
  if(clipped)
    msgs.push('El nombre no cabe ni en dos líneas y se ha recortado. Marca «Nombre condensado» o quita el QR.');
  if(mod && mod<4)
    msgs.push('El QR quedó con módulo de '+mod+' px. Por debajo de 4 px muchos teléfonos no lo leen.');
  if(fsFoot < mm(1.35))
    msgs.push('El texto de pie queda por debajo de 1.35 mm y la térmica lo cerrará.');
  if(tagW > fullW)
    msgs.push('El lema no cabe a ancho completo. Acorta el aviso legal o quita el QR.');
